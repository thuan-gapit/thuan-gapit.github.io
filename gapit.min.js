...
(function () {
    ...
    var visibilityContent = function ($this, payload) {
    ...
    var insertModal = function ($this) {
        ...
        (function () {
            var getDataSuccess = function (data) {
                ...
                document.querySelector(`#${$this.element.id} iframe`).contentWindow.postMessage(
                    {
                        ...
                        payload: $this.options.formConfig.success
                    },
                    '*'
                )
                document.querySelector(`#${$this.element.id} .gapit-content`).style.extend({
                ...
                if ($this.options.formConfig.show_option.display_frequency.show_again) {
                    setCookie('isSubmmitFormSignup', true, $this.options.formConfig.show_option.display_frequency.show_again * 24 * 60);
                }
                else {
                    ...
                    switch (type) {
                        ...
                        case windownMessType.postDataDefault:
                            getDataSuccess({
                                zalo: {
                                    user: payload.userId,
                                    oaid: $this.options.formConfig.zalo.oaid,
                                },
                                form_id: $this.options.formId
                            });
                            break;
                        ...
                        case windownMessType.resizeIframe:
                            visibilityContent($this, payload);

                            if ($this.options.formId && $this.options.isShowInline !== true) {
                                _goq.push(['trackEvent', 'form_view', { form_id: $this.options.formId }]);
                            }
        ...
        insertModalInline = function ($this) {
        ...
        attachEventListeners = function ($this) {
            ...
            var showFrequency = function (fn, displayFrequency) {
                ...
                if (!isSubmmit || isSubmmit === false) {
                    fn();
                    if (show_again > 0) {
                        setCookie('GPPopupLookUp', true, show_again * 24 * 60);
                    }
                    else {
                        setCookie('GPPopupLookUp', true);
                    }
                }
            }
            ...
            var { show_option = {} } = $this.options.formConfig;
            var { display_timing = {} } = show_option;
            if (display_timing.show_immediately === true) {
                fnShow.target = function () {
                    fadeIn.display(() => {
                        onShow();
                    }, 1000)
                };
                showFollowTarget(fnShow.target, show_option);
            }
            else {
    ...
    var presentProcess = async function (options) {
        ...
        // create html
        var renderIframe = renderIframeContent(form.outerHTML, options, zaloDiv?.outerHTML);
        options.extend({
    ...
    GapitScript.reRender = async function (options) {
        options.extend({
            ...formType,
            getConfig: `${window.GapitUrl.config}/?userId=${options.userId}&formId=${options.formId}`,
        })
