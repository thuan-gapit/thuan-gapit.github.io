...
(function () {
        ...
        (function () {
            var getDataSuccess = function (data) {
                ...
                document.querySelectorAll(`#${$this.options.elementId} iframe`)[0].contentWindow.postMessage(
                    {
                        type: messageType.messageSuccess,
                        payload: {
                            title: $this.options.formConfig.mesSuccess,
                            description: $this.options.formConfig.mesSuccessDescription
                        }
                    },
                    '*'
                )
            ...
            window.onmessage = function (e) {
                ...
                if (!isEmptyObject(payload) &&
                    ...
                    switch (type) {
                        case messageType.postDataDefault:
                            getDataSuccess({
                                ...payload,
    ...
    var createHtmlPopup = function (formHTML, config, options) {
        ...
        var templateContentForm = `
            ...
            ${options.formConfig.zalo.isExist === true ? `<script src="https://sp.zalo.me/plugins/sdk.js"></script>` : ''}
            <script>
                var sharedGapitZalo = function (data) {
                    if (data) {
                        userId = data.userId;
                        window.top.postMessage({
                            type: '${messageType.postZaloFollow}',
                            payload: {
                                userId
                            }
                        }, '*')
                ...
                (function() {
                    document.querySelector('form').addEventListener('submit', (e) => {
                        ...
                        window.top.postMessage({
                            type: '${messageType.postDataDefault}',
                            payload: data
                        }, '*')
                    });
                    window.onmessage = function (e) {
                        var {payload, type} = e.data;
                        if (type == '${messageType.messageSuccess}') {
    ...
    var presentProcess = async function (options, url) {
        ...
        bf.render(form, config.form);
        options.formConfig = config;
        var zalo = {};
        var checkIsHasFollow = function (objectCheck) {
            while (!isEmptyObject(objectCheck)) {
                if (objectCheck.type == 'zalo_follow') {
                    zalo.isExist = true;
                    zalo.oaid = objectCheck.default;
                    return;
                }
                for (var key in objectCheck.properties) {
                    checkIsHasFollow(objectCheck.properties[key]);
                }
                objectCheck = {}
            }
        }
        checkIsHasFollow(options.formConfig.form);
        options.formConfig.zalo = zalo;

        options.src = createHtmlPopup(form.outerHTML, config, options);
        return options;
    }
    ...
    GapitScript.init = async function (options) {
        ...
        options = await presentProcess(options, 'http://localhost:8181/config');
...
(function (d, s, id) {
    ...
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = 'http://localhost:8181/css/gapit.min.css';
