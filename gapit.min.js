...
(function () {
    ...
    var setCookie = function (c_name, value, expireminutes) {
        var exdate = new Date();
    ...
    var insertModal = function ($this) {
        ...
        attachEventListeners = function ($this) {
            ...
            var showFrequency = function (fn, displayFrequency) {
                var isSubmmit = getCookie('isSubmmit');
                var { show_again = 0 } = displayFrequency;
                if (isSubmmit === false) {
                    fn();
                    if (show_again > 0) {
                        setCookie('isSubmmit', true, show_again * 24 * 60);
                    }
                    else {
                        setCookie('isSubmmit', true);
                    }
                }
            }
            ...
            var { showOption = {} } = $this.options.formConfig;
            var { display_timing = {} } = showOption;
            if (display_timing.show_immediately === true) {
                showFollowTarget(fadeIn.display(() => {
                    onShow();
                }, 1000), showOption);
            }
            else {
                var { visibilitychange = false, second_after_page_load = 0, after_scroll_percent = 0 } = showOption.loading_delay ? showOption.loading_delay : {};
                if (visibilitychange === true) {
                    showFollowTarget(fadeIn.visibilitychange(onShow), showOption);
                }
                if (second_after_page_load && Number(second_after_page_load) > 0) {
                    showFollowTarget(fadeIn.display(() => {
                        onShow();
                    }, Number(second_after_page_load) * 1000), showOption);
                }
                if (after_scroll_percent && after_scroll_percent > 0) {
                    document.addEventListener("scroll", () => {
                        var percent = getScrollPercent();
                        if (percent == after_scroll_percent) {
                            showFollowTarget(fadeIn.display(() => {
                                onShow();
                            }, 1000), $this.options.formConfig.showOption);
                        }
                    });
                }
            }
    ...
    GapitScript.reRender = async function (options) {
        options.extend({
            ...formType,
            getConfig: `https://dev-thuannv.gapone.vn/push/signup-form/?userId=8&formId=6`,
        })
    ...
    GapitScript.init = async function (options) {
        options.extend({
            ...formType,
            getConfig: `https://dev-thuannv.gapone.vn/push/signup-form/?userId=${options.userId}&formId=${options.formId}`
        })
        options = await presentProcess(options);
...
(function (d, s, id) {
    ...
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = 'https://dev-thuannv.gapone.vn/static/css/gapit.min.css';
