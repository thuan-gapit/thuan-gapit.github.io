...
(function () {
    ...
    var getSchemaFromType = function (obj, type) {
        ...
        var checker = function (object) {
            while (!isEmptyObject(object)) {
                if (object.type == type) {
                    result.isExist = true;
                    result.oaid = object.default;
                    break;
                }
                for (var key in object.properties) {
                    checker(object.properties[key]);
                }
                object = {}
            }
        }
    ...
    var insertModal = function ($this) {
        ...
        (function () {
            var getDataSuccess = function (data) {
                ...
                document.querySelectorAll(`#${$this.options.elementId} iframe`)[0].contentWindow.postMessage(
                    {
                        type: windownMessType.messageSuccess,
                        payload: $this.options.formConfig.messageSuccess
                    },
                    '*'
                )
                document.getElementById('gapit-content').style.extend({
                    height: "200px",
                    width: "450px",
                    visibility: "visible"
                });
                    ...
                    switch (type) {
                        ...
                        case windownMessType.resizeIframe:
                            ...
                            if ($this.options.formId && $this.options.isShowInline !== true) {
                                //_goq.push(['trackEvent', 'form_view', { form_id: formId }]);
                            }
    ...
    var insertModalInline = function ($this) {
        // add modal to site inline
        $this.content.append($this.iframe);
        document.getElementById($this.options.elementId).append($this.content);
        window.onmessage = function (e) {
            const { payload, type } = e.data;
            if (!isEmptyObject(payload) &&
                !isEmptyObject(type)
    ...
    GapitModal.prototype.init = function () {
        if (this.options.isShowInline === true) {
    ...
    var renderPopupContent = function (formHTML, options) {
        var formContent;
        var { title, description, style, zalo, messageSuccess, formType, css } = options.formConfig;
        if (options.isShowInline == true && options.isShowThanks == true) {
            formContent = '<div class="form-thanks"> <h3 class="success-title">' + messageSuccess.title + '</h3> <div class="success-des">' + messageSuccess.description + "</div> </div>";
        }
        else {
            if (formType === options.formType1) {
                formContent = `
                    <div class="main">
                        <h3 class="title">${title}</h3>
                        <div class="desciption">${description}</div>
                        <div class="form" id="${options.formId}">${formHTML}</div>
                    </div>
                `;
            }
            else if (formType === options.formType2) {
                formContent = `
        ...
        var backgroundForm =
            `background-color: ${style.backgroundColor ? style.backgroundColor : '#ffffff'}`;
        if (style.backgroundImage) {
            backgroundForm = `background-image: url(${style.backgroundImage})`;
        }
        var headIframe = `
        <head>
                <style>
                    ...
                    .success-title {
                        font-size: 30px;
                        margin: 10px 0px;
                        color: ${style.messageTitleColor ? style.messageTitleColor : 'inherit'};
                    }

                    .success-des {
                        font-size: 16px;
                        color: ${style.messageDesColor ? style.messageDesColor : 'inherit'};
                    }
                    ...
                    .form-control:hover, .form-control:focus, .form-control:active, button:hover, button:focus, button:active, input:hover, input:focus, input:active, select:hover, select:focus, select:active {
                        ...
                        border-color: ${style.btnSubmitColor ? style.btnSubmitColor : 'inherit'};
                    }
                    ...
                    *[type="submit"] {
                        ...
                        border: none;
                        margin-bottom: 10px;
                        background-color: ${style.btnSubmitColor ? style.btnSubmitColor : 'inherit'};
                        font-size: 16px;
                    ...
                    .title {
                        ...
                        line-height: 1.2;
                        margin-top: 0px;
                        color: ${style.titleColor ? style.titleColor : 'inherit'}
                    }

                    .desciption {
                        font-size: 16px;
                        margin-bottom: 10px;
                        color: ${style.descriptionColor ? style.descriptionColor : 'inherit'}
                    }
                ...
                (function() {
                    window.onmessage = function (e) {
                        ...
                        switch (type) {
                            case "${windownMessType.reRenderIframe}": 
                                var {headIframe, src, isLoadElement, isLoadZalo} = payload;
                                if (isLoadElement == true) {
                                    document.querySelector('html').innerHTML = src;
                                    if (isLoadZalo == true) {
                                        loadPage();
                                    }
                                    window.top.postMessage({
                            ...
                            case "${windownMessType.messageSuccess}": 
                                ...
                                '<div class="form-thanks"> <h3  class="success-title">' + payload.title + '</h3> <div class="success-des">' + payload.description + "</div> </div>";
                    ...
                    window.addEventListener('DOMContentLoaded', function (e) {
                        window.top.postMessage({
                            type: '${windownMessType.resizeIframe}',
                            payload: {
                                height: document.body.scrollHeight.offsetHeight + ${(zalo.isExist == true && options.isShowThanks !== true) ? 20 : 0}
                            }
                        }, '*')
                    });
                }());
    ...
    var presentProcess = async function (options) {
        ...
        var zalo = getSchemaFromType(options.formConfig.form, 'zalo_follow');
        if (!isEmptyObject(zalo)) {
            options.formConfig.zalo = {
                isExist: true,
                oaid: zalo.default
            };
        }
        if (options.formConfig.formType == formType.formType2) {
            options.width = 650;
        }

        // create html
        var renderIframe = renderPopupContent(form.outerHTML, options);
        options.extend({
    ...
    GapitScript.init = async function (options) {
        options.extend({
            ...formType,
            getConfig: 'https://dev-thuannv.gapone.vn/config'
        })
        options = await presentProcess(options);
        preOptions.form = { ...options.formConfig.form };
    ...
    GapitScript.reRender = async function (options) {
        ...
        options = await presentProcess(options);
        var { zalo, form } = options.formConfig;
        document.querySelectorAll(`#${options.elementId} iframe`)[0].contentWindow.postMessage(
            {
                type: windownMessType.reRenderIframe,
                payload: {
                    src: options.src,
                    headIframe: options.headIframe,
                    isLoadElement: !isEqualsJson(form, preOptions.form),
        ...
        document.querySelectorAll(`#${options.elementId} iframe`)[0].contentWindow.postMessage(
            {
                ...
                payload: {
                    src: options.src,
                    headIframe: options.headIframe,
                    isLoadElement: !isEqualsJson(options.formConfig.form, preOptions.form)
                }
            },
            '*'
        )
